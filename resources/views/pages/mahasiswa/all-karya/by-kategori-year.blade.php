...
<div class="container-fluid px-4 py-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h2 class="fw-bold text-gradient-primary mb-2">Karya: {{ $kategori->nama_kategori }} ({{ $tahun }})</h2>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0">
                    <li class="breadcrumb-item"><a href="{{ route('mahasiswa.akarya.byKategori', $kategori) }}">Kategori</a></li>
                    <li class="breadcrumb-item active" aria-current="page">{{ $tahun }}</li>
                </ol>
            </nav>
        </div>
        <a href="{{ route('mahasiswa.akarya.byKategori', $kategori) }}" class="btn btn-secondary rounded-pill px-4 shadow-sm">
    ...
    <div class="row mb-4">
        ...
        <div class="col-md-8">
            <div class="card shadow-sm mb-4" style="border-radius: 12px; border: none;">
                <div class="card-body p-3">
                    <h6 class="mb-3 text-muted"><i class="bi bi-filter-circle me-2"></i>Filter Tahun</h6>
                    <div class="d-flex flex-wrap gap-2">
                        <a href="{{ route('mahasiswa.akarya.byKategori', $kategori) }}" 
                           class="btn btn-sm btn-{{ !request()->has('jenis_karya') ? 'light' : 'light' }} rounded-pill px-3 shadow-sm">
                            <i class="bi bi-collection me-2"></i>Semua
                        </a>
                        @foreach($years as $year)
                        <a href="{{ route('mahasiswa.akarya.byKategoriAndYear', [$kategori, $year]) }}" 
                           class="btn btn-sm btn-{{ $year == $tahun ? 'primary' : 'light' }} rounded-pill px-3 shadow-sm">
        ...
        <div class="col-md-4">
            <div class="card shadow-sm" style="border-radius: 12px; border: none;">
                <div class="card-body p-3">
                    <h6 class="mb-3 text-muted"><i class="bi bi-tags me-2"></i>Filter Jenis Karya</h6>
                    <div class="d-flex flex-wrap gap-2">
                        <a href="{{ route('mahasiswa.akarya.byKategoriAndYear', [$kategori, $tahun]) }}" 
                           class="btn btn-sm btn-{{ !request()->has('jenis_karya') ? 'primary' : 'light' }} rounded-pill px-3 shadow-sm">
                            <i class="bi bi-collection me-2"></i>Semua
                        </a>
                        <a href="{{ route('mahasiswa.akarya.byKategoriAndYear', [$kategori, $tahun, 'jenis_karya' => 'video']) }}" 
                           class="btn btn-sm btn-{{ request()->get('jenis_karya') == 'video' ? 'primary' : 'light' }} rounded-pill px-3 shadow-sm">
                            <i class="bi bi-camera-reels me-2"></i>Video
                        </a>
                        <a href="{{ route('mahasiswa.akarya.byKategoriAndYear', [$kategori, $tahun, 'jenis_karya' => 'gambar']) }}" 
                           class="btn btn-sm btn-{{ request()->get('jenis_karya') == 'gambar' ? 'primary' : 'light' }} rounded-pill px-3 shadow-sm">
                            <i class="bi bi-images me-2"></i>Gambar
                        </a>
                        <a href="{{ route('mahasiswa.akarya.byKategoriAndYear', [$kategori, $tahun, 'jenis_karya' => 'dokumen']) }}" 
                           class="btn btn-sm btn-{{ request()->get('jenis_karya') == 'dokumen' ? 'primary' : 'light' }} rounded-pill px-3 shadow-sm">
                ...
                <div class="card-body">
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        <span class="badge rounded-pill bg-light text-dark">
                            ...
                            @endphp
                            {{ $karya->tanggal_publish->translatedFormat('d M Y') }}
                        </span>
                        <a href="{{ route('mahasiswa.akarya.show', $karya) }}" class="btn btn-sm btn-primary rounded-pill px-3">
        ...
        <div class="col-12">
            <div class="card shadow-sm" style="border-radius: 12px; border: none;">
                <div class="card-body text-center py-5">
                    ...
                    <a href="{{ route('mahasiswa.akarya.byKategori', $kategori) }}" class="btn btn-primary rounded-pill px-4 mt-3">
    ...
    <div class="d-flex justify-content-center mt-4">
        <nav aria-label="Page navigation">
            {{ $karyas->onEachSide(1)->links('pagination::bootstrap-5') }}
        </nav>
    </div>
    @endif
</div>
